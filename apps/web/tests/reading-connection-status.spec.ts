...
test("verified Hardcover carries from metadata to reading accounts and settings", async ({
  page,
}, testInfo) => {
  const errors: string[] = [];
  ...
  for (const provider of ["Goodreads", "StoryGraph", "Hardcover"]) {
    ...
    const heading = section.locator(".reading-account-heading > span").first();
  ...
  await page
    ...
    .screenshot({ path: testInfo.outputPath("reading-onboarding-mobile.png") });
  await page.reload();
  await expect(hardcover.locator(".connection-state")).toHaveText("Connected");
